...
POSTS = [
	{},
    {'title': 'zero', 'content': 'zero', 'job': 'tea', 'username': 'a0','password': '123456','vip': 'True'},
...
class PostResource(Resource):
    ...
    def get(self, post_id):
        ...
        return {'title': post.title, 'content': post.content,'job': post.job}
    ...
    def put(self, post_id):
        ...
        Post.update(title=json_data['title'], content=json_data['content'], job = json_data['job']).where(Post.id == post_id).execute()
        # POSTS[post_id] = post
        return post, 201
...
class PostList(Resource):
    ...
    def get(self):
    	posts = []
        # 查询数据，需要做分页查询
        raw_posts = Post.select().limit(1000)
#=========== 变量：批量添加所有字节
